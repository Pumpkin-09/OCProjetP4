import re
from Vue.Vue_verification import AffichageVerification as AV
...
class AffichageJoueur:
    ...
    def nouveau_joueur(settings):
        ...
        numero_ine = AV.verification_input(" - ", lambda numero_ine: re.match(r"^AB\d{5}$", numero_ine))
        if numero_ine in settings:
            print(f"le numéro INE {numero_ine} est déjà présent dans la base de données.")
            return None
        nom = AV.verification_input("Veuillez saisir le Nom du joueur:\n - ", lambda nom: nom != "")
        prenom = AV.verification_input("Veuillez saisir le Prénom du joueur:\n - ", lambda prenom: prenom != "")
        print("Veuillez saisir la Date de Naissance du joueur:")
        date_de_naissance = AV.verification_input("(date au format JJ/MM/AAAA)\n - ", AV.verification_date)
    ...
    def recherche_joueur(tournoi, donnees_joueur, numero_ine):
        ...
        while True:
            ...
            joueur_ine = AV.verification_input(" - ", lambda joueur_ine: re.match(r"^(AB\d{5}|0)$", joueur_ine))
